...
describe('AuthController - Security Tests', () => {
    ...
    describe('POST /auth/register', () => {
        ...
        it('should call authService.register with the provided DTO', async () => {
            const expectedResult = { user: sanitizedUser(), accessToken: 'token', refreshToken: 'rtoken' };
        ...
        it('should propagate ConflictException for duplicate email', async () => {
            ...
            mockAuthService.register.mockRejectedValue(new ConflictException('El email ya está registrado'));
        ...
        it('should propagate ConflictException for duplicate username', async () => {
            const { ConflictException } = await import('@nestjs/common');
            mockAuthService.register.mockRejectedValue(new ConflictException('El username ya está en uso'));
    ...
    describe('POST /auth/login', () => {
        ...
        it('should propagate UnauthorizedException for wrong credentials (generic error)', async () => {
            ...
            await expect(controller.login(validCredentials as any)).rejects.toThrow(UnauthorizedException);
        });
    ...
    describe('SECURITY: Route Guard Configuration', () => {
        it('GET /auth/me should be protected by JwtAuthGuard', () => {
            ...
            const hasGuard =
                (classGuards && classGuards.some((g: any) => g === JwtAuthGuard || g?.name === 'JwtAuthGuard')) ||
                (methodGuards && methodGuards.some((g: any) => g === JwtAuthGuard || g?.name === 'JwtAuthGuard'));
