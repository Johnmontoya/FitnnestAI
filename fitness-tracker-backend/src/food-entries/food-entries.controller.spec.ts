...
describe('FoodEntriesController - Security Tests', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [{ provide: FoodEntriesService, useValue: mockFoodEntriesService }],
    })
  ...
  describe('SECURITY: GET /food-entries - Scoped to Authenticated User', () => {
    ...
    it('User B cannot retrieve User A entries via findAllUser (scoped by JWT userId)', async () => {
      ...
      await controller.findAllUser(
        makeReq(USER_B) as any,
        '2024-01-15T00:00:00',
        '2024-01-15T23:59:59',
      );

      // Service called with User B's ID, not User A's
      expect(mockFoodEntriesService.findAllUser).toHaveBeenCalledWith(
        USER_B,
        expect.any(String),
        expect.any(String),
      );
      expect(mockFoodEntriesService.findAllUser).not.toHaveBeenCalledWith(
        USER_A,
        expect.anything(),
        expect.anything(),
      );
  ...
  describe('SECURITY: GET /food-entries/:id - IDOR Prevention', () => {
    ...
    it('should propagate NotFoundException if entry does not belong to the requesting user', async () => {
      ...
      mockFoodEntriesService.findOne.mockRejectedValue(
        new NotFoundException('Entrada no encontrada'),
      );

      await expect(controller.findOne(makeReq(USER_B) as any, 'food-uuid-001')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('SECURITY: PUT /food-entries/:id - IDOR Prevention', () => {
    it('should pass authenticated userId to service when updating', async () => {
      ...
      expect(mockFoodEntriesService.update).toHaveBeenCalledWith('food-uuid-001', USER_A, {
        name: 'Updated',
      });
    });

    it('should reject if entry belongs to a different user (NotFoundException)', async () => {
      mockFoodEntriesService.update.mockRejectedValue(
        new NotFoundException('Entrada no encontrada'),
      );
  ...
  describe('SECURITY: DELETE /food-entries/:id - IDOR Prevention', () => {
    ...
    it('should reject deletion of entries belonging to a different user', async () => {
      mockFoodEntriesService.remove.mockRejectedValue(
        new NotFoundException('Entrada no encontrada'),
      );

      await expect(controller.remove(makeReq(USER_B) as any, 'food-uuid-001')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
